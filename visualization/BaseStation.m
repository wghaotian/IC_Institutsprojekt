classdef BaseStation < SimulationsObject
    ...
    methods     
        ...
        %% Serve
        function [map,BS]=serveCS(BS,CS_ind,map,last_evnt)
            global time;
            if (BS.sleepMode==0) % active
                ...
                map.CS_List(CS_ind).act_time=time;
            elseif (BS.sleepMode==1) % idle
                BS.serveList=[BS.serveList,CS_ind];
                evnt.last_evnt=last_evnt;
                evnt.time=time;
                evnt.name='active';
                evnt.type='BS';
                evnt.ind=BS.ind;
...
%                 map.eventList=push(map.eventList,new_evnt);
 ...
 %                       obj.prev_SM=1;
                    ...
                    if (obj.numSleep(nxt_SM)) % Next sleep mode determined
                        ...
                        if time && obj.Delay<conf.time_eps && obj.prev_SM
        ...
        function [obj,map]=observe(obj,map,time,last_evnt)
            served=[];
            flag=false;
            for I=1:length(obj.serveList)
                ind=obj.serveList(I);
                [map.CS_List(ind),map,flag1]=map.CS_List(ind).observe(map,time,last_evnt);
                flag=flag||flag1;
